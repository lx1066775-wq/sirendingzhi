...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
        if (selectedTemplateId && TEMPLATES[selectedTemplateId]) {
            ...
            const templateData = JSON.parse(JSON.stringify(TEMPLATES[selectedTemplateId])); // Deep copy
            
            // If user selected Mode C but template is A/B, we just load it as is for now.
            // User can click "Translate" later.
            // But we update metadata to match request if compatible
            if (request.mode !== 'C') {
            ...
            const result = await generateItinerary(apiKey, request);
            setData(result);
        }
    } catch (err: any) {
  ...
  const handleTranslate = async () => {
    ...
    setError(null);
    try {
        const translatedData = await translateItinerary(apiKey, data);
        setData(translatedData);
        setRequest(prev => ({ ...prev, mode: 'C' })); // Update mode to C to reflect change
    } catch (err: any) {
